...
public class CreateDeclarationTemplateTask extends CustomTask{
    ...
    protected void deleteAndCreate() {
        ...
        Bennu.getInstance().getDeclarationTemplateSet().stream().flatMap(t -> t.getDeclarationFileSet().stream()).forEach(
        ...
        if (Bennu.getInstance().getDeclarationTemplateSet().isEmpty()) {
            ...
            String descriptionFormat = "Declaração de Matrícula do ano letivo %4$s";

            String filenameFormat = "%s_declaracao_%s_%s_%s.pdf";
            SignatureFieldSettings
                    signatureFieldSetttings = new SignatureFieldSettings(100, 300, 500, 200,
            "signatureField", 1);
            ...
            DeclarationTemplate declarationTemplate = new DeclarationTemplate(name, displayName, resolveHtmlFromFile(filename),
             pt, filenameFormat, titleFormat, descriptionFormat, signatureFieldSetttings, studentGroup.or(academicRequestsGroup));
            ...
            declarationTemplate = new DeclarationTemplate(name, displayName, resolveHtmlFromFile(filename), en, filenameFormat,
             titleFormat, descriptionFormat,  signatureFieldSetttings,studentGroup.or(academicRequestsGroup));
            declarationTemplate.setBennuFirstTimeRegistrationTemplate(Bennu.getInstance());

            signatureFieldSetttings = new SignatureFieldSettings(100, 300, 500, 200, "signatureField",
                    2);
            ...
            descriptionFormat = "Declaração de Transportes Sub23";
            ...
            titleFormat, descriptionFormat, signatureFieldSetttings , academicRequestsGroup);
            ...
            new DeclarationTemplate(name, displayName, resolveHtmlFromFile(filename), pt,filenameFormat, titleFormat, descriptionFormat, signatureFieldSetttings , academicRequestsGroup);
        }
    }
